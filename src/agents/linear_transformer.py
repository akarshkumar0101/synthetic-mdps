...
def main():
    ...
    forward_parallel = jax.vmap(partial(net.apply, method=net.forward_parallel), in_axes=(None, 0))
    forward_recurrent = jax.vmap(partial(net.apply, method=net.forward_recurrent), in_axes=(None, 0, 0))
    ...
    rng, _rng = jax.random.split(rng)
    obs_ = jax.tree_map(lambda x: x[0], dict(obs=obs, act_p=action, rew_p=reward))
    params = net.init(_rng, obs_, method=net.forward_parallel)

    print('params shape')
    print(jax.tree_map(lambda x: x.shape, params))

    # ------ PARALLEL FORWARD ------
    obs_ = jax.tree_map(lambda x: x, dict(obs=obs, act_p=action, rew_p=reward))
    logits1, values1 = forward_parallel(params, obs_)

    # ------ RECURRENT FORWARD ------
    obs_ = jax.tree_map(lambda x: rearrange(x, 'b t ... -> t b ...'), dict(obs=obs, act_p=action, rew_p=reward))
