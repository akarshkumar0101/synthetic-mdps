...
def sample_batch_segments_from_dataset(_rng, dataset, batch_size, n_segs, seg_len):
    ...
    # batch_prv = jax.tree.map(lambda x: x[n, t-1, ...], dataset)
    # batch_prv = {f"{k}_prv": v for k, v in batch_prv.items()}
    # batch_nxt = jax.tree.map(lambda x: x[n, t+1, ...], dataset)
...
def main(args):
    ...
    for i_iter in pbar:
        ...
        if args.env_id is not None and (i_iter==0 or i_iter==n_iters):
            ...
            def agent_forward(obs):
                obs = data_utils.transform_obs(obs, transform_params)
