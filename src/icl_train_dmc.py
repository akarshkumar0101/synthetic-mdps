...
def main(args):
    ...
    def iter_test(train_state, _rng):
        batch = sample_test_batch(_rng)
    ...
    @jax.jit
    def iter_test_multiple(train_state, rng):
        return jax.lax.scan(iter_test, train_state, split(rng, 10))

    @jax.jit
    def iter_train_multiple(train_state, rng):
        return jax.lax.scan(iter_train, train_state, split(rng, 100))
