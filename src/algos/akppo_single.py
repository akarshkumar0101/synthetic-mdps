...
class PPO:
    def __init__(self, agent, env, env_params, tx=None, *,
        ...
        self.agent_forward_recurrent = partial(self.agent.apply, method=self.agent.forward_recurrent)
        self.agent_forward_recurrent = jax.vmap(self.agent_forward_recurrent, in_axes=(None, 0, 0))
    ...
    def rollout_step(self, carry, _):
        ...
        # env
        rng, _rng = split(rng)
        obs_n, env_state_n, rew, done, info = jax.vmap(self.env.step, in_axes=(0, 0, 0, None))(split(_rng, self.n_envs),
                                                                                               env_state, act,
                                                                                               env_params)
    ...
    def init_agent_env(self, rng):
        ...
        rng, _rng = split(rng)
        obs, env_state = jax.vmap(self.env.reset, in_axes=(0, None))(split(_rng, self.n_envs), env_params)
