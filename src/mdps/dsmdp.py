...
class Transition:
    ...
    def sample_params(self, rng):
        ...
        return {"trans/trans_matrix": trans_matrix}

    def __call__(self, rng, state, action, params):
        logits = params['trans/trans_matrix'][action, state, :]
...
class Observation:
    ...
    def sample_params(self, rng):
        ...
        return {"obs/obs_matrix": obs_matrix}

    def __call__(self, rng, state, params):
        return params['obs/obs_matrix'][state] + self.std * jax.random.normal(rng, (self.d_obs,))
...
class Reward:
    ...
    def sample_params(self, rng):
        ...
        return {"rew/rew_matrix": rew_matrix}

    def __call__(self, rng, state, params):
        rew = params['rew/rew_matrix'][state] + self.std * jax.random.normal(rng, ())
        if self.sparse:
...
class Done:
    ...
    def sample_params(self, rng):
        ...
        return {"done/done_matrix": done_matrix}

    def __call__(self, rng, state, params):
        done = params['done/done_matrix'][state] + self.std * jax.random.normal(rng, ())
        thresh = jax.scipy.stats.norm.ppf(self.sparse_prob)
        return done<thresh
